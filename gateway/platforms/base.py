...
async def cache_image_from_url(url: str, ext: str = ".jpg", retries: int = 2, auth_headers: dict | None = None) -> str:
    """
    ...
    async with httpx.AsyncClient(
        ...
        for attempt in range(retries + 1):
            try:
                ...
                if auth_headers:
                    _hdrs.update(auth_headers)
                response = await client.get(url, headers=_hdrs)
